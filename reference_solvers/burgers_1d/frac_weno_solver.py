...
try:
    from .frac_utils import frac_BDF_coeff
except ImportError:
    from frac_utils import frac_BDF_coeff
...
def frac_burgers_equation_solver(u0: np.ndarray, x: np.ndarray, T: float, nu: float, 
                            bc_func: Callable, Nt: int, al:float, du0:np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Solve the Burgers' equation using WENO scheme, Lax-Friedrichs flux splitting, and convolution quadrature.
    ...
    # here t may be different from the original code
    t = np.linspace(0, T, Nt+1)
    dt = t[1]-t[0]
    w = frac_BDF_coeff(2*Nt, al)
    w = w.reshape(-1, 1)
    ...
    try:
        ...
        for n  in range(Nt+1):
            if n==0: continue 
            RHS = -np.sum(u[n-1::-1, :]*w[1:n+1], axis=0)+np.sum(w[0:n+1])*u[0] +np.sum(w[0:n+1].flatten()*t[n::-1])*du0+dt**al*rhs(u[n-1]) 
            u[n] = RHS/w[0]
            u[n] = bc_func(u[n])
            if n % (Nt // 10) == 0:
                print(f"Step {n}, t={t[n]:.6f}, dt={dt:.6f}, Max u: {np.max(u[n]):.6f}, Min u: {np.min(u[n]):.6f}") # while t[n] < T and n < Nt:
            # dt = 
