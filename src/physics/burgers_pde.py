...
class BurgersPDE(TimeFracCaputoDiffusionWaveTwoDimPDE):
    def __init__(self, cfg: DictConfig, device: torch.device):
        ...
        self.beta = getattr(cfg.pde, "beta", 1.0)
        self.nu = getattr(cfg.pde, "nu", 0.01 / np.pi)
        self.datafile = getattr(cfg.pde, "datafile", None)
        self.datafile = os.path.join(get_original_cwd(), self.datafile)
    ...
    def exact(self, points: Tensor) -> Tensor:
        """
        for burgers equation, we use points in the data
        """
        return torch.from_numpy(self.data['u'].T.flatten()).to(self.device)
