...
class DWForward(PINN):
    ...
    def residual(self, net, points_all):
        ...
        dt, dxx = self.frac_diff(net, points) 
        losses['in'] = dt-dxx*self.lam/(self.k*self.k*np.pi*np.pi) #N*1 
    ...
    def frac_diff(self, net, points):
        ...
        if config.method == 'MC-I':
            ...
            dt0 = torch.autograd.grad(outputs=val0, inputs=t0, grad_outputs=torch.ones_like(val0),retain_graph=True, create_graph=True)[0][:,0:1]#N*1
    ...
    def exact(self, points):
        points = points.detach().cpu().numpy() #N*2 
        t = points[:, 0:1] #N*1 
        x = points[:, 1:2] #N*1
    ...
    def evaluator(self, net, count):
        ...
        exact = self.exact(points_tensor) #N*1 
